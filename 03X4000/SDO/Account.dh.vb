'------------------------------------------------------------------------------
' <copyright file="account.sdo.vb" company="Microsoft">
'     Copyright (c) Microsoft Corporation.  All rights reserved.
' </copyright>
'------------------------------------------------------------------------------

Option Strict Off
Option Explicit On
Imports Solomon.Kernel
Module sdoACCOUNT
	
	Public Class Account
			Inherits SolomonDataObject
    < _
    DataBinding(PropertyIndex:=0, StringSize:=10) _
    > _
    Public Property Acct() As String

        Get
            Return Me.GetPropertyValue("Acct")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Acct", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=1, StringSize:=2) _
    > _
    Public Property AcctType() As String

        Get
            Return Me.GetPropertyValue("AcctType")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("AcctType", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=2, StringSize:=16) _
    > _
    Public Property Acct_Cat() As String

        Get
            Return Me.GetPropertyValue("Acct_Cat")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Acct_Cat", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=3, StringSize:=1) _
    > _
    Public Property Acct_Cat_SW() As String

        Get
            Return Me.GetPropertyValue("Acct_Cat_SW")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Acct_Cat_SW", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=4) _
    > _
    Public Property Active() As Short

        Get
            Return Me.GetPropertyValue("Active")
        End Get

        Set(ByVal setval As Short)
            Me.SetPropertyValue("Active", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=5, StringSize:=10) _
    > _
    Public Property ClassID() As String

        Get
            Return Me.GetPropertyValue("ClassID")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("ClassID", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=6, StringSize:=10) _
    > _
    Public Property ConsolAcct() As String

        Get
            Return Me.GetPropertyValue("ConsolAcct")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("ConsolAcct", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=7) _
    > _
    Public Property Crtd_DateTime() As Integer

        Get
            Return Me.GetPropertyValue("Crtd_DateTime")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("Crtd_DateTime", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=8, StringSize:=8) _
    > _
    Public Property Crtd_Prog() As String

        Get
            Return Me.GetPropertyValue("Crtd_Prog")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Crtd_Prog", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=9, StringSize:=10) _
    > _
    Public Property Crtd_User() As String

        Get
            Return Me.GetPropertyValue("Crtd_User")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Crtd_User", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=10, StringSize:=4) _
    > _
    Public Property CuryId() As String

        Get
            Return Me.GetPropertyValue("CuryId")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("CuryId", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=11, StringSize:=30) _
    > _
    Public Property Descr() As String

        Get
            Return Me.GetPropertyValue("Descr")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Descr", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=12, StringSize:=1) _
    > _
    Public Property Employ_Sw() As String

        Get
            Return Me.GetPropertyValue("Employ_Sw")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Employ_Sw", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=13) _
    > _
    Public Property LUpd_DateTime() As Integer

        Get
            Return Me.GetPropertyValue("LUpd_DateTime")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("LUpd_DateTime", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=14, StringSize:=8) _
    > _
    Public Property LUpd_Prog() As String

        Get
            Return Me.GetPropertyValue("LUpd_Prog")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("LUpd_Prog", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=15, StringSize:=10) _
    > _
    Public Property LUpd_User() As String

        Get
            Return Me.GetPropertyValue("LUpd_User")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("LUpd_User", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=16) _
    > _
    Public Property NoteID() As Integer

        Get
            Return Me.GetPropertyValue("NoteID")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("NoteID", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=17, StringSize:=2) _
    > _
    Public Property RatioGrp() As String

        Get
            Return Me.GetPropertyValue("RatioGrp")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("RatioGrp", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=18, StringSize:=30) _
    > _
    Public Property S4Future01() As String

        Get
            Return Me.GetPropertyValue("S4Future01")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("S4Future01", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=19, StringSize:=30) _
    > _
    Public Property S4Future02() As String

        Get
            Return Me.GetPropertyValue("S4Future02")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("S4Future02", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=20) _
    > _
    Public Property S4Future03() As Double

        Get
            Return Me.GetPropertyValue("S4Future03")
        End Get

        Set(ByVal setval As Double)
            Me.SetPropertyValue("S4Future03", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=21) _
    > _
    Public Property S4Future04() As Double

        Get
            Return Me.GetPropertyValue("S4Future04")
        End Get

        Set(ByVal setval As Double)
            Me.SetPropertyValue("S4Future04", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=22) _
    > _
    Public Property S4Future05() As Double

        Get
            Return Me.GetPropertyValue("S4Future05")
        End Get

        Set(ByVal setval As Double)
            Me.SetPropertyValue("S4Future05", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=23) _
    > _
    Public Property S4Future06() As Double

        Get
            Return Me.GetPropertyValue("S4Future06")
        End Get

        Set(ByVal setval As Double)
            Me.SetPropertyValue("S4Future06", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=24) _
    > _
    Public Property S4Future07() As Integer

        Get
            Return Me.GetPropertyValue("S4Future07")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("S4Future07", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=25) _
    > _
    Public Property S4Future08() As Integer

        Get
            Return Me.GetPropertyValue("S4Future08")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("S4Future08", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=26) _
    > _
    Public Property S4Future09() As Integer

        Get
            Return Me.GetPropertyValue("S4Future09")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("S4Future09", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=27) _
    > _
    Public Property S4Future10() As Integer

        Get
            Return Me.GetPropertyValue("S4Future10")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("S4Future10", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=28, StringSize:=10) _
    > _
    Public Property S4Future11() As String

        Get
            Return Me.GetPropertyValue("S4Future11")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("S4Future11", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=29, StringSize:=10) _
    > _
    Public Property S4Future12() As String

        Get
            Return Me.GetPropertyValue("S4Future12")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("S4Future12", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=30, StringSize:=1) _
    > _
    Public Property SummPost() As String

        Get
            Return Me.GetPropertyValue("SummPost")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("SummPost", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=31, StringSize:=6) _
    > _
    Public Property UnitofMeas() As String

        Get
            Return Me.GetPropertyValue("UnitofMeas")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("UnitofMeas", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=32, StringSize:=1) _
    > _
    Public Property Units_SW() As String

        Get
            Return Me.GetPropertyValue("Units_SW")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("Units_SW", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=33, StringSize:=30) _
    > _
    Public Property User1() As String

        Get
            Return Me.GetPropertyValue("User1")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("User1", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=34, StringSize:=30) _
    > _
    Public Property User2() As String

        Get
            Return Me.GetPropertyValue("User2")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("User2", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=35) _
    > _
    Public Property User3() As Double

        Get
            Return Me.GetPropertyValue("User3")
        End Get

        Set(ByVal setval As Double)
            Me.SetPropertyValue("User3", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=36) _
    > _
    Public Property User4() As Double

        Get
            Return Me.GetPropertyValue("User4")
        End Get

        Set(ByVal setval As Double)
            Me.SetPropertyValue("User4", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=37, StringSize:=10) _
    > _
    Public Property User5() As String

        Get
            Return Me.GetPropertyValue("User5")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("User5", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=38, StringSize:=10) _
    > _
    Public Property User6() As String

        Get
            Return Me.GetPropertyValue("User6")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("User6", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=39) _
    > _
    Public Property User7() As Integer

        Get
            Return Me.GetPropertyValue("User7")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("User7", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=40) _
    > _
    Public Property User8() As Integer

        Get
            Return Me.GetPropertyValue("User8")
        End Get

        Set(ByVal setval As Integer)
            Me.SetPropertyValue("User8", setval)
        End Set

    End Property

    < _
    DataBinding(PropertyIndex:=41, StringSize:=1) _
    > _
    Public Property ValidateID() As String

        Get
            Return Me.GetPropertyValue("ValidateID")
        End Get

        Set(ByVal setval As String)
            Me.SetPropertyValue("ValidateID", setval)
        End Set

    End Property

	End Class
Public bAccount As Account = New Account, nAccount As Account = New Account
End Module
